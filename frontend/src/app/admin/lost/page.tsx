...
export default function AdminLostItemsPage() {
  ...
  const fetchLostItems = async () => {
    setLoading(true);
    try {
      const resp = await api.get("/admin/lost");
      setItems(resp.data || []);
    } catch (err) {
      console.error(err);
      setItems(mockLost);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await api.patch(`/admin/lost/${editingItem.id}`, updates);
  ...
  const handleDeleteItem = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/admin/lost/${deletingItem.id}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E5E2DC] pb-5">
        <div>
          <h1 className="font-display text-2xl font-bold text-[#1E2022]">Lost Items Inventory</h1>
          <p className="text-xs text-[#72787E] mt-0.5">Admin management for all reported lost items in the database.</p>
        </div>
      ...
      <ConfirmModal
        ...
        message={`This will permanently delete this lost item report ("${deletingItem?.title}") and its vector embeddings. This cannot be undone.`}
...
const mockLost = [
  { id: "lost_101", title: "Black Leather Wallet with ID Cards", category: "Wallets & Bags", location: "Central Library", user_name: "Alex Rivera", status: "active", image_url: "https://images.unsplash.com/photo-1627123424574-724758594e93?w=500&q=80" },
  { id: "lost_103", title: "Sage Green Hydro Flask Water Bottle", category: "Personal Belongings", location: "Science Building", user_name: "Sarah Connor", status: "active", image_url: "https://images.unsplash.com/photo-1602143407151-7111542de6e8?w=500&q=80" },
];
