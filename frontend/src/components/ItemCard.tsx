    ...
    <div className="bg-[#FAF8F5] border border-[#E5E2DC] rounded-lg p-4 space-y-3">
      ...
      <div className="w-full bg-[#EAE7E1] h-2.5 rounded-full overflow-hidden flex">
        <div
          ...
          style={{ width: `${Math.round(imageSim * 60)}%` }}
          title={`Image (60% weight): ${Math.round(imageSim * 100)}%`}
        />
        <div
          className="bg-[#D97706] h-full transition-all duration-500"
          style={{ width: `${Math.round(textSim * 25)}%` }}
          title={`Text (25% weight): ${Math.round(textSim * 100)}%`}
        />
        <div
          className="bg-[#4A5056] h-full transition-all duration-500"
          style={{ width: `${Math.round(contextScore * 15)}%` }}
          title={`Context (15% weight): ${Math.round(contextScore * 100)}%`}
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-[11px] font-mono pt-1">
        <div className="bg-white border border-[#E5E2DC] rounded p-1.5">
          <span className="block text-[#72787E]">Visual (60%)</span>
          <span className="font-bold text-[#2E4A3E]">{Math.round(imageSim * 100)}%</span>
        </div>
        <div className="bg-white border border-[#E5E2DC] rounded p-1.5">
          <span className="block text-[#72787E]">Text (25%)</span>
          <span className="font-bold text-[#D97706]">{Math.round(textSim * 100)}%</span>
        </div>
        <div className="bg-white border border-[#E5E2DC] rounded p-1.5">
          <span className="block text-[#72787E]">Context (15%)</span>
