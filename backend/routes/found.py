...
async def get_found_item(item_id: str):
    ...
    if db is not None:
        try:
            doc = await db["items"].find_one({"_id": ObjectId(item_id), "type": "found", "deleted": {"$ne": True}})
            if doc:
                doc["id"] = str(doc["_id"])
...
async def delete_found_item(item_id: str, current_user=Depends(get_current_user)):
    """
    User endpoint to soft-delete their found item.
    ...
    if db is not None:
        ...
        item = await db["items"].find_one({"_id": obj_id, "type": "found", "deleted": {"$ne": True}})
        if not item:
